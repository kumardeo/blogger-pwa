...
|     `manifest.display`      | `string` |     `'standalone'`     |    no    | Display mode of the app                           |
|   `manifest.orientation`    | `string` |      `'natural'`       |    no    | Orientation of the app                            |
|      `manifest.scope`       | `string` |         `'/'`          |    no    | Scope of the app                                  |
|    `manifest.start_url`     | `string` |         `'/'`          |    no    | Url to open when app is launched                  |
|    `manifest.shortcuts`     | `array`  |      `undefined`       |    no    | List of all shortcuts of the app                  |
|          `origin`           | `string` |      `undefined`       |    no    | The origin of your blog (required when using CDN) |
|         `oneSignal`         | `object` |  `{ enabled: false }`  |    no    | OneSignal config                                  |
...
9. Also set `oneSignal.enabled` to `true`.
...
2. Add the following javascript just above to `</body>`:
   ...
   <script type="module">
     ...
     (({ button: t, onInstall: n }) => {
       ...
       button:
         document.getElementById("app_install_button") ||
         Object.assign(
